...
function nearestStopLabel(nextStops: NextStop[]): string {
  const valid = nextStops.filter((s) => s.stop != null);
  if (valid.length === 0) return dimText("—");
  const first = valid[0];
  const mins = first.waitingTime;
  if (mins <= 1) return `at ${stopColor(first.stop!.name)}`;
  return `near ${stopColor(first.stop!.name)} ${dimText(`(${mins} mín)`)}`;
}

export function formatBusOverview(
  route: string,
  buses: BusLocation[],
  lastUpdate: string
): string {
  ...
  const maxHeadsign = Math.max(...buses.map((b) => b.headsign.length));
  ...
  for (const bus of buses) {
    ...
    const hs = bus.headsign.padEnd(maxHeadsign);
    const location = nearestStopLabel(bus.nextStops);
    lines.push(`  ${letter}  ${dimText("→")} ${stopColor(hs)}  ${location}`);
  }

  return lines.join("\n");
}

export function formatBusDetail(
  bus: BusLocation,
  lastUpdate: string,
  lastStop?: string
): string {
  const valid = bus.nextStops.filter((s) => s.stop != null);
  ...
  for (let i = 0; i < valid.length; i++) {
    const ns = valid[i];
    const mins = ns.waitingTime;
    const time = mins <= 1 ? dimText("< 1 mín") : dimText(`~${mins} mín`);
    const isNext = i === 0;
    const marker = isNext ? chalk.cyan("▸") : dimText("○");
    const name = isNext
      ? chalk.bold.cyan(ns.stop!.name.padEnd(22))
      : stopColor(ns.stop!.name.padEnd(22));
...
export function formatStops(stops: Stop[]): string {
  ...
  for (const stop of stops) {
    const routes = stop.routes.length > 0
      ? ` ${stop.routes.map((r) => routeColor(r)).join(dimText(", "))}`
      : "";
...
export function formatStop(stop: Stop & { streetView?: { iframeUrl: string } | null }): string {
  const lines = [
    ...
    `  ${dimText("Routes:")}  ${stop.routes.length > 0 ? stop.routes.map((r) => routeColor(r)).join(", ") : "none"}`,
  ];
...
export function formatAlerts(alerts: Alert[]): string {
  ...
  for (const alert of alerts) {
    const routes = alert.routes.map((r) => routeColor(r)).join(", ");
    lines.push(`  ${chalk.red("!")} ${chalk.bold(alert.title)}`);
    lines.push(`    ${dimText("Routes:")} ${routes}  ${dimText("Cause:")} ${alert.cause.toLowerCase().replace(/_/g, " ")}`);
...
export function formatGeocode(results: GeoResult[]): string {
  ...
  for (const r of results) {
    lines.push(`  ${chalk.bold(r.name)}`);
    lines.push(`    ${dimText(r.address)}  ${dimText(`(${r.type})`)}  ${dimText(`${r.lat}, ${r.lon}`)}`);
  }
...
export function formatTrips(trips: TripItinerary[]): string {
  ...
  for (let i = 0; i < shown.length; i++) {
    ...
    const transferLabel = transfers === 0
      ? chalk.green("Direct")
      : `${transfers} transfer${transfers !== 1 ? "s" : ""}`;

    lines.push(
      `  ${chalk.bold(start)} → ${chalk.bold(end)}  ${dimText(dur)}  ${transferLabel}`
    );
